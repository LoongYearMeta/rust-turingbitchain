...
use crate::hashes::{self, sha256, Hash, sha256d};
...
impl Transaction {
    ...
    /// Computes the input hash and script hash separately
    pub fn input_and_scripts_hash(&self) -> (sha256::Hash, sha256::Hash) {
        // Step 1: Encode inputs (TXID, VOUT, Sequence)
        let mut input_data = Vec::new();
        for input in &self.input {
            input.previous_output.txid.consensus_encode(&mut input_data).expect("engines don't error");
            input.previous_output.vout.consensus_encode(&mut input_data).expect("engines don't error");
    ...
    pub fn output_hash(&self) -> sha256::Hash {
        // Step 3: Encode outputs (Value, LockingScript)
        let mut output_data: Vec<u8> = Vec::new();
        for output in &self.output {
            output.value.consensus_encode(&mut output_data).expect("engines don't error");
